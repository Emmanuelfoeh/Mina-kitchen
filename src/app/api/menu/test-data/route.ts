...
export async function POST() {
  ...
  try {
    ...
    // Seed against the default tenant (categories/items are tenant-scoped).
    const tenant = await db.tenant.findFirst({
      where: { subdomain: 'default' },
    });
    if (!tenant) {
      return NextResponse.json(
        { success: false, error: 'Default tenant not found. Seed it first.' },
        { status: 400 }
      );
    }
    ...
    const mainDishes = await db.menuCategory.upsert({
      where: { tenantId_name: { tenantId, name: 'Main Dishes' } },
      update: {},
      create: {
        name: 'Main Dishes',
    ...
    const soups = await db.menuCategory.upsert({
      where: { tenantId_name: { tenantId, name: 'Soups' } },
      update: {},
      create: {
        name: 'Soups',
    ...
    await db.menuCategory.upsert({
      where: { tenantId_name: { tenantId, name: 'Sides' } },
      update: {},
      create: {
        name: 'Sides',
    ...
    const starters = await db.menuCategory.upsert({
      where: { tenantId_name: { tenantId, name: 'Starters' } },
      update: {},
      create: {
        name: 'Starters',
